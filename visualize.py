    ...
    with PdfPages(path / 'plots.pdf') as pdf:
        for sample in tqdm(dataset):
            ...
            if result.exists():
                ...
                # get inputs
                orig_image = prettify(sample, kspace=True)
                adv_image = prettify(adv_sample, kspace=True)

                # reconstruct outputs
                with torch.no_grad():
                    orig_output_pt = model(sample)
                    orig_output = prettify(orig_output_pt)
                    adv_output = prettify(model(adv_sample))
